...
export default function Quizzes({ domain }: { domain: string }) {
  ...
  const [state, dispatch, isPending] = useActionState(createQuizAction, initialState);
  ...
  const [quizParams, setQuizParams] = useState({ topic: '', difficulty: '' });
    ...
    addHistoryItem({
        ...
        details: {
            ...
            questions: quiz.map((q, i) => ({
                question: q.question,
                selectedAnswer: q.options[answers[i]],
